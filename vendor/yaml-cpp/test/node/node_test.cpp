...
namespace YAML {
    namespace {
        ...
        TEST(NodeTest, UndefinedConstNodeWithFallback) {
            Node node;
            const Node& cn = node;
            EXPECT_THROW(cn["undefined"].as<int>(3), YAML::KeyNotFound);
        }
        ...
        TEST(NodeTest, StdArray) {
            ...
            Node node;
            node["evens"] = evens;
            std::array<int, 5> actualEvens =
                node["evens"].as<std::array<int, 5>>();
        ...
        TEST(NodeTest, StdMap) {
            ...
            std::map<int, int> actualSquares =
                node["squares"].as<std::map<int, int>>();
        ...
        TEST(NodeTest, StdMapWithCustomAllocator) {
            ...
            Node node;
            node["squares"] = squares;
            CustomMap<int, int> actualSquares =
                node["squares"].as<CustomMap<int, int>>();
            EXPECT_EQ(squares, actualSquares);
        }
...
#pragma warning(disable : 4800)
        ...
        TEST(NodeTest, KeyNodeExitsScope) {
            ...
            for (Node::const_iterator it = node.begin(); it != node.end();
                 ++it) {
                (void)it;
            }
        }
        ...
        TEST(NodeTest, AccessNonexistentKeyOnConstNode) {
            ...
            EXPECT_THROW(other["5"], YAML::KeyNotFound);
        }
