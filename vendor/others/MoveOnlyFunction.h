/**
 * @brief C++ still (< 23) doesn't have a move only function
 * https://en.cppreference.com/w/cpp/utility/functional/move_only_function/move_only_function
 */

// src: https://github.com/ofats/any_invocable
...
namespace ofats {
    ...
    namespace any_detail {
        ...
        using buffer =
            std::aligned_storage_t<sizeof(void*) * 2, alignof(void*)>;
        ...
        struct handler_traits {
            ...
            struct small_handler : handler_base<small_handler<T>> {
                ...
                static void move(storage& dst, storage& src) noexcept {
                    create(dst, std::move(*static_cast<T*>(
                                    static_cast<void*>(&src.buf_))));
                    destroy(src);
                }

                static R call(const storage& s, ArgTypes... args) {
                    return std::invoke(*static_cast<T*>(static_cast<void*>(
                                           &const_cast<storage&>(s).buf_)),
            ...
            template <class T>
            using handler =
                std::conditional_t<is_small_object_v<T>, small_handler<T>,
                                   large_handler<T>>;
        };
        ...
        class any_invocable_impl {
            template <class T>
            using handler = typename any_detail::handler_traits<
                R, ArgTypes...>::template handler<T>;

            using storage = any_detail::storage;
            using action = any_detail::action;
            using handle_func = void (*)(any_detail::action,
                                         any_detail::storage*,
                                         any_detail::storage*);
           ...
           protected:
            ...
            friend bool operator==(const any_invocable_impl& f,
                                   std::nullptr_t) noexcept {
                return !f;
            }
            friend bool operator==(std::nullptr_t,
                                   const any_invocable_impl& f) noexcept {
                return !f;
            }
            friend bool operator!=(const any_invocable_impl& f,
                                   std::nullptr_t) noexcept {
            ...
            friend void swap(any_invocable_impl& lhs,
                             any_invocable_impl& rhs) noexcept {
                lhs.swap(rhs);
            }
        ...
        template <class AI, class F, bool noex, class R, class FCall,
                  class... ArgTypes>
        using can_convert = std::conjunction<
            ...
            std::bool_constant<(
                !noex || std::is_nothrow_invocable_r_v<R, FCall, ArgTypes...>)>,
...
#define __OFATS_ANY_INVOCABLE(cv, ref, noex, inv_quals)                        \
    ...
    class any_invocable<R(ArgTypes...) cv ref noexcept(noex)> final            \
        ...
        using base_type =                                                      \
            any_detail::any_invocable_impl<R, noex, ArgTypes...>;              \
                                                                               \
       public:                                                                 \
...
using MoveOnlyFunction = ofats::any_invocable<arg>;